...
export default function Home({ params }: { params: { lang: Lang } }) {
  ...
  return <Layout lang={lang}><main>
    <section id="home" className="bg-gradient-to-br from-white via-mist to-sky"><div className="container py-16 lg:py-24"><div className="grid gap-10 lg:grid-cols-[1.15fr_.85fr] lg:items-center"><div><p className="eyebrow">海外教員・研究者受入ワンストップサポート</p><h1 className="mt-4 text-4xl font-extrabold leading-tight text-navy md:text-6xl">海外教員・研究者等の受入を、来日前から着任後まで支援します</h1><p className="mt-4 text-lg font-semibold text-slate-600">One-stop support for hosting international faculty, researchers, and guests at Ritsumeikan.</p><p className="mt-6 text-xl text-ink">COE・ビザ、宿泊、航空券、関空到着時の支援、住居、銀行口座開設、生活立ち上げなど、海外からの教員・研究者・短期ゲスト等を受け入れる際に必要となる周辺業務を、クレオテックが学内窓口となり、外部パートナーと連携しながら支援します。</p><p className="mt-4 text-base text-slate-600">Creotech supports Ritsumeikan staff and faculty by coordinating COE and visa support, accommodation, flight and airport arrival support, housing, banking, and settling-in assistance through internal coordination and external partners.</p><div className="mt-8 flex flex-wrap gap-4"><a className="btn btn-primary" href="#contact">相談する <span className="ml-2 text-sm opacity-80">Contact us</span></a><a className="btn btn-secondary" href="#timeline">着任時期から確認する <span className="ml-2 text-sm opacity-70">Check the timeline</span></a></div></div><div className="card p-7"><div className="rounded-3xl bg-navy p-7 text-white"><p className="text-sm font-bold uppercase tracking-widest text-white/65">Creotech role</p><h2 className="mt-3 text-3xl font-extrabold">学内調整窓口として連携します</h2><p className="mt-4 text-white/80">Creotech introduces, connects, organises, and coordinates with external partners. It does not guarantee approvals, reservations, cancellations, or payments.</p></div></div></div></div></section>
