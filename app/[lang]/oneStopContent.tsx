...
export function FeesPage() { return <Section eyebrow="Fees" title="費用の考え方" en="How costs are handled"><div className="page-intro-visual page-intro-visual-fees mb-7" role="img" aria-label="費用区分と相談見積を想起させる画像エリア（後日差し替え）"><span>Cost category image area</span></div><div className="grid gap-5 md:grid-cols-2">{fees.map(([title, desc]) => <article className="card p-6 md:p-7" key={title}><h2 className="text-2xl font-extrabold text-navy">{title}</h2><p className="mt-3">{desc}</p></article>)}</div><div className="mt-6 grid gap-4 rounded-3xl bg-navy p-7 text-white"><p>航空券について、本人負担の場合は本人が事前に決済します。学内部署負担の場合は、MYKと学内部署が請求書により精算します。COE・ビザ支援費用については、本人負担または学内部署負担を選択できます。</p><p className="text-white/80">予約、支払い、変更、キャンセル条件は、各ホテル、旅行会社、外部サービス提供者等の規定に従います。クレオテックは、学内窓口として候補紹介、情報整理、外部パートナーとの連携支援を行います。</p></div></Section>; }

export function FaqPage() { return <Section eyebrow="FAQ" title="よくある質問" en="Frequently asked questions"><div className="page-intro-visual page-intro-visual-faq mb-7" role="img" aria-label="問い合わせ前の確認事項を案内する画像エリア（後日差し替え）"><span>Guidance image area</span></div><div className="grid gap-4 md:grid-cols-2">{faqs.map(([q, a]) => <article className="card p-5 md:p-6" key={q}><h2 className="text-xl font-extrabold text-navy">{q}</h2><p className="mt-3">{a}</p></article>)}</div></Section>; }
