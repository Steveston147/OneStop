...
const imageSlots = {
  hero: { src: '/images/hero-ritsumeikan-campus.jpg', enabled: false, alt: '立命館キャンパスのイメージ', label: 'Campus image' },
  services: { src: '/images/services-support.jpg', enabled: false, alt: '支援内容のイメージ', label: 'Support visual' },
  timeline: { src: '/images/timeline-planning.jpg', enabled: false, alt: '着任準備スケジュールのイメージ', label: 'Planning visual' },
  accommodation: { src: '/images/accommodation-banner.jpg', enabled: false, alt: '宿泊施設のイメージ', label: 'Accommodation visual' },
  fees: { src: '/images/fees-planning.jpg', enabled: false, alt: '費用確認のイメージ', label: 'Cost planning visual' },
  faq: { src: '/images/faq-guide.jpg', enabled: false, alt: '質問確認のイメージ', label: 'FAQ visual' },
  visitors: { src: '/images/visitors-campus-life.jpg', enabled: false, alt: '海外からの来訪者受入支援のイメージ', label: 'Visitor support visual' },
  contact: { src: '/images/contact-guide.jpg', enabled: false, alt: '問い合わせ案内のイメージ', label: 'Contact visual' },
} as const;
...
export function VisualArea({
  ...
  const size = compact ? 'min-h-24 aspect-[5/2]' : 'min-h-32 aspect-[16/6]';

  return (
    <div className={`relative overflow-hidden rounded-3xl border border-line bg-gradient-to-br from-white via-mist to-sky ${size} ${className}`}>
      {image.enabled ? <img src={image.src} alt={image.alt} className="h-full w-full object-cover" /> : null}
      {!image.enabled ? (
        <div className="absolute inset-0 grid place-items-center p-5 text-center">
          <div>
            <span className="mx-auto block h-1.5 w-16 rounded-full bg-accent/50" />
            <p className="mt-3 text-xs font-extrabold uppercase tracking-[.18em] text-slate-500">{image.label}</p>
            <p className="mt-1 text-sm font-semibold text-slate-500">画像追加予定 / Image placeholder</p>
          </div>
        </div>
      ) : null}
    </div>
