...
export function Section({ eyebrow, title, en, children }: { eyebrow: string; title: string; en: string; children: React.ReactNode }) {
  return <section className="section even:bg-mist"><div className="container"><p className="eyebrow">{eyebrow}</p><h2 className="mt-2 max-w-4xl text-3xl font-extrabold leading-tight text-navy md:text-5xl">{title}</h2><p className="mb-8 mt-2 max-w-3xl text-base font-semibold leading-relaxed text-slate-500">{en}</p>{children}</div></section>;
}
...
export function ServicesPage() {
  return <><Section eyebrow="Who we support" title="立命館大学の学内関係者向けサービスです。" en="For Ritsumeikan host offices, faculty, international visitors, and accompanying family members."><AudienceCards /></Section><Section eyebrow="Support menu" title="支援内容" en="Support coordinated by Creotech and external partners"><div className="grid gap-5 lg:grid-cols-2">{services.map((s) => <article className="card overflow-hidden" key={s.ja}><div className="service-card-visual" role="img" aria-label={`${s.ja}の将来写真エリア`}><span className="visual-chip">{s.en}</span></div><div className="p-6 md:p-7"><h2 className="text-2xl font-extrabold text-navy">{s.ja}</h2><p className="text-sm font-semibold text-slate-500">{s.en}</p><p className="mt-4">{s.desc}</p><dl className="mt-5 grid gap-3 text-sm"><Meta k="推奨開始時期" v={s.timing}/><Meta k="主な連携先・担当" v={s.partner}/><Meta k="費用メモ" v={s.cost}/></dl></div></article>)}</div></Section></>;
}

export function TimelinePage() { return <Section eyebrow="Timeline" title="時期別の準備目安" en="Plan backwards from the appointment date"><div className="page-intro-visual page-intro-visual-timeline mb-7" role="img" aria-label="着任準備とスケジュール確認を想起させる将来写真エリア"><span className="visual-chip">Preparation schedule</span></div><div className="grid gap-5 md:grid-cols-2">{timelines.map(([title, items]) => <article className="card p-6 md:p-7" key={title}><h2 className="text-3xl font-extrabold text-navy">{title}</h2><ul className="mt-5 grid gap-3">{items.map(item => <li className="rounded-xl bg-mist p-4 font-semibold" key={item}>✓ {item}</li>)}</ul></article>)}</div><p className="mt-6 rounded-2xl border border-gold/30 bg-light-gold p-5 font-semibold text-navy">直前のご相談にも可能な範囲で対応しますが、COE・ビザ、宿泊、航空券、関空到着支援等の一部サービスを提供できない場合があります。できるだけ早めにご相談ください。</p></Section>; }

export function AccommodationPage({ lang }: { lang: Lang }) { return <Section eyebrow="Accommodation" title="候補比較のポイント" en="Accommodation options introduced for later replacement with official links"><div className="grid gap-6">{hotels.map((h) => <article className="card overflow-hidden md:grid md:grid-cols-[240px_1fr] lg:grid-cols-[280px_1fr]" key={h.name}><div className="hotel-image-placeholder grid aspect-[16/9] min-h-44 place-items-center p-8 text-center text-sm font-bold text-slate-500 md:aspect-auto"><span className="visual-chip">{h.area}</span></div><div className="p-6 md:p-7"><h2 className="text-2xl font-extrabold text-navy">{h.name}</h2><p className="text-sm font-semibold text-slate-500">{h.en}</p><p className="mt-3">{h.features}</p><dl className="mt-5 grid gap-2 text-sm md:grid-cols-2"><Meta k="Area" v={h.area}/><Meta k="Nearest station" v={h.station}/><Meta k="Access" v={h.campus}/><Meta k="Price range" v={h.price}/><Meta k="Family stay" v={h.family}/><Meta k="Long stay" v={h.long}/></dl><Link className="btn btn-secondary mt-5 w-full sm:w-auto" href={`/${lang}/contact`}>公式予約リンク（後日更新）</Link></div></article>)}</div><p className="mt-6 rounded-2xl border border-line bg-white p-5 font-semibold">掲載ホテルは宿泊候補の紹介です。予約、支払い、変更、キャンセルは、各ホテルの公式サイトまたは案内に従って利用者ご自身で行ってください。料金、空室状況、条件は時期により変動します。</p></Section>; }

export function FeesPage() { return <Section eyebrow="Fees" title="見積・実費・負担者の整理" en="How costs are handled"><div className="page-intro-visual page-intro-visual-fees mb-7" role="img" aria-label="費用区分と相談見積を想起させる将来写真エリア"><span className="visual-chip">Quotation based support</span></div><div className="grid gap-5 md:grid-cols-2">{fees.map(([title, desc]) => <article className="card p-6 md:p-7" key={title}><h2 className="text-2xl font-extrabold text-navy">{title}</h2><p className="mt-3">{desc}</p></article>)}</div><div className="mt-6 grid gap-4 rounded-3xl bg-navy p-7 text-white"><p>航空券について、本人負担の場合は本人が事前に決済します。学内部署負担の場合は、MYKと学内部署が請求書により精算します。COE・ビザ支援費用については、本人負担または学内部署負担を選択できます。</p><p className="text-white/80">予約、支払い、変更、キャンセル条件は、各ホテル、旅行会社、外部サービス提供者等の規定に従います。クレオテックは、学内窓口として候補紹介、情報整理、外部パートナーとの連携支援を行います。</p></div></Section>; }

export function FaqPage() { return <Section eyebrow="FAQ" title="相談前に確認したいこと" en="Frequently asked questions"><div className="page-intro-visual page-intro-visual-faq mb-7" role="img" aria-label="問い合わせ前の確認事項を案内する将来写真エリア"><span className="visual-chip">Questions and answers</span></div><div className="grid gap-4 md:grid-cols-2">{faqs.map(([q, a]) => <article className="card p-5 md:p-6" key={q}><h2 className="text-xl font-extrabold text-navy">{q}</h2><p className="mt-3">{a}</p></article>)}</div></Section>; }

export function VisitorsPage() { return <Section eyebrow="For International Visitors" title="Before and after arrival support" en="Basic information for incoming guests"><div className="card overflow-hidden"><div className="visitors-hero-visual" role="img" aria-label="Campus life support future image area"><span className="visual-chip">Campus life support</span></div><div className="p-6 md:p-8"><p className="text-lg">This page provides basic information for international faculty, researchers, short-term guests, and accompanying family members who will come to Ritsumeikan. Creotech works with your host office and external partners to support preparation before arrival and settling in after arrival.</p><div className="mt-6 grid gap-3 md:grid-cols-2 lg:grid-cols-3">{['Before coming to Japan','COE and visa support','Accommodation','Flight and airport arrival support','First days in Japan','Payment and costs','Contact your host office'].map(x => <div className="rounded-xl bg-mist p-4 font-bold text-navy" key={x}>{x}</div>)}</div><p className="mt-6 rounded-2xl border border-gold/25 bg-light-gold p-5 font-bold text-navy">Please contact your host office at Ritsumeikan first. Some services must be requested through the host department or office.</p></div></div></Section>; }

export function ContactPage() { return <Section eyebrow="Contact" title="初期相談に必要な情報" en="Visual-ready inquiry form; submission integration can be added later"><div className="page-intro-visual page-intro-visual-contact mb-7" role="img" aria-label="問い合わせ案内のための将来写真エリア"><span className="visual-chip">Inquiry guidance</span></div><form className="card grid gap-5 p-6 md:p-8"><p className="rounded-2xl border border-gold/30 bg-light-gold p-4 font-semibold text-navy">このフォームはPhase 1 MVPの画面見本です。実際の送信連携は後続フェーズで追加します。お急ぎの場合は学内担当窓口へご連絡ください。</p><div className="grid gap-5 md:grid-cols-2">{contactFields.map((label) => <label className="grid gap-2 font-bold text-navy" key={label}>{label}<input className="input" /></label>)}</div><label className="grid gap-2 font-bold text-navy">その他相談内容 / Additional comments<textarea className="input min-h-36" /></label><button className="btn btn-cta w-full sm:w-fit" type="button">送信連携は後日追加 / Submission coming later</button></form></Section>; }
