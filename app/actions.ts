...
const applicantCategoryLabels: Record<string, Record<Lang, string>> = {
  'Host Office / Faculty': {
    ja: '受入部署の担当者・教員',
    en: 'Host office staff or faculty member',
  },
  'Faculty / Researcher / Guest': {
    ja: '来日予定の教員・研究者・ゲスト',
    en: 'Incoming faculty, researcher, or guest',
  },
  'Family Member': {
    ja: '同行家族',
...
const hostInstitutionLabels: Record<string, Record<Lang, string>> = {
  ...
  'Ritsumeikan Asia Pacific University': {
    ja: '立命館アジア太平洋大学（APU）',
    en: 'Ritsumeikan Asia Pacific University (APU)',
  },
  'Ritsumeikan Junior & Senior High School (Nagaokakyo)': {
    ja: '立命館中学校・高等学校（長岡京）',
    en: 'Ritsumeikan Junior & Senior High School (Nagaokakyo)',
  },
  'Ritsumeikan Uji Junior & Senior High School': {
    ja: '立命館宇治中学校・高等学校',
    en: 'Ritsumeikan Uji Junior & Senior High School',
  },
  'Ritsumeikan Moriyama Junior & Senior High School': {
    ja: '立命館守山中学校・高等学校',
    en: 'Ritsumeikan Moriyama Junior & Senior High School',
  },
  'Ritsumeikan Primary School': {
    ja: '立命館小学校',
    en: 'Ritsumeikan Primary School',
  },
  Other: {
    ja: 'その他',
    en: 'Other',
...
const requestedServiceLabels: Record<string, Record<Lang, string>> = {
  'COE / Visa guidance': {
    ja: 'COE・ビザ関連支援',
    en: 'COE and visa guidance',
  },
  'Accommodation support': {
    ja: '宿泊・住居支援',
    en: 'Accommodation and housing',
  },
  'Flight support': {
    ja: '航空券に関する支援',
    en: 'Flight support',
  ...
  'Airport meet and assist': {
    ja: '空港到着・待ち合わせ支援',
    en: 'Airport arrival support',
  },
  'City office support': {
    ja: '市役所手続き支援',
    en: 'City-office support',
  },
  'Bank account support': {
    ja: '銀行口座支援',
    en: 'Bank-account support',
  },
  'Mobile / SIM / Internet': {
    ja: '携帯電話・SIM・通信支援',
    en: 'Mobile, SIM, and internet support',
  },
  'Family support': {
    ja: '同行家族支援',
    en: 'Accompanying family support',
  },
  'Campus orientation': {
    ja: 'キャンパス・校内案内',
    en: 'Campus or school orientation',
  },
  Other: {
    ja: 'その他',
    en: 'Other',
...
export async function submitRequest(_previousState: ActionState, formData: FormData): Promise<ActionState> {
  ...
  const institutionValue = text(formData, 'host_institution');
  const institution = localize(hostInstitutionLabels, institutionValue, lang);
  ...
  const stayPeriod = formatStayPeriod(
    text(formData, 'planned_arrival_date'),
    text(formData, 'planned_departure_date'),
    lang,
  );
  ...
  const rows: Array<[string, string]> = lang === 'ja'
    ? [
        ...
        ['利用者区分', role],
        ['受入機関・所属', institution],
        ['受入部署・所属詳細', text(formData, 'host_department')],
        ['学内・校内担当者', text(formData, 'host_contact')],
        ['氏名', fullName],
        ['メールアドレス', email],
        ['現在の居住国', text(formData, 'current_country')],
        ['国籍', text(formData, 'nationality')],
        ['受入・滞在予定期間', stayPeriod],
        ['同行家族人数', text(formData, 'family_members')],
        ['希望する支援', localizedServices.join('、')],
    ...
    : [
        ...
        ['Host department or affiliation', text(formData, 'host_department')],
        ['Host contact', text(formData, 'host_contact')],
        ['Full name', fullName],
        ['Email address', email],
        ['Current country of residence', text(formData, 'current_country')],
        ['Nationality', text(formData, 'nationality')],
        ['Expected period of stay', stayPeriod],
        ['Accompanying family members', text(formData, 'family_members')],
        ['Requested support', localizedServices.join(', ')],
  ...
  const htmlRows = rows
    .map(([label, value]) => `<tr><th style="padding:10px 12px;text-align:left;vertical-align:top;border-bottom:1px solid #e5e7eb;background:#f8fafc;width:34%;">${escapeHtml(label)}</th><td style="padding:10px 12px;vertical-align:top;border-bottom:1px solid #e5e7eb;white-space:pre-wrap;">${escapeHtml(display(value, lang))}</td></tr>`)
    .join('');
